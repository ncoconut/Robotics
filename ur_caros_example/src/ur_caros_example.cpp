 ...
 public:
  ...
  bool planner_rrt(double x, double y, double z, RPY<double> rpy) {
    ...
    do {
      ...
      rw::math::Transform3D<double> baseTtool_desired_rand(pos_rand,
                                                           rpy.toRotation3D());
      // Biased solution towards goal area
      std::vector<rw::math::Q> solutions_rand =
          solver->solve(baseTtool_desired_rand, state);
      rw::math::Q q_rand;
...
int main(int argc, char **argv) {
  ...
  rw::math::Q q_ini(6, -0.384695, -1.08527, 1.37812, -0.309625, 1.56999, 0.0178859);
  robot.setQ(q_ini);
